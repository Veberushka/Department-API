...
class DepartmentServiсe:
    ...
    @classmethod
    def departments_to_dicts(cls,departaments,include_employees=False):
        ...
        for departament in departaments:
            departament_dict = departament.to_dict()
        
            if include_employees and hasattr(departament, 'employees'):
                departament_dict['employees'] = [
                    emp.to_dict() for emp in departament.employees
                ]
            else:
                departament_dict['employees'] = []
               
            all_departments_dicts.append(departament_dict)
    ...
    async def get_by_id_with_children(cls,id,depth,include_employees):
        ...
        all_department=await cls._dao.get_all(include_employees)
    ...
    async def update_parent_id_with_name(cls,id,parent_id,name):
        ...
        if parent_id:
            all_department=cls.departments_to_dicts(await cls._dao.get_all())
        ...
        try:
            result=await cls._dao.update({'id':id},parent_id=parent_id,name=name)
            obj = result[0]
    ...
    async def delete_by_id(cls,id,mode,reassign_to_department_id):
        ...
        if mode =='cascade':
            ...
            await cls._dao.update({'id__in':child_id}, parent_id=reassign_to_department_id)
            await cls._dao.update_employees_department(id, reassign_to_department_id)
            await cls._dao.delete(id=id)
    ...
    async def add_department(cls,name,parent_id: int|None=None):
        ...
        existing=None
        if parent_id:
            existing= await cls._dao.get_all(parent_id=parent_id,name=name)
        if existing:
            raise Error(
            status_code=409,
            detail=f"Департрамент '{name}' уже существует в подчинении {parent_id}")
        try:
            new_department = await cls._dao.add(name=name,parent_id=parent_id)
            return {
